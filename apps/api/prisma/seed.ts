...
const emptySchema = { fields: [] } satisfies { fields: AttributeField[] };
...
} satisfies Record<string, { fields: AttributeField[] }>;
      ...
      where: {
        type: {
          in: [
            ...
            NotificationType.BUNDLE_RESULT,
...
async function upsertDemoUsers(passwordHash: string) {
  ...
  const renterEmails = demoUsers
    .filter((user) => user.role === UserRole.RENTER)
    .map((user) => user.email);
  ...
  for (const parent of taxonomy) {
    const createdParent = await prisma.category.create({
      data: {
        ...
        attributesSchema: schemaFor(parent.slug) as Prisma.InputJsonValue,
      },
    });
    ...
    for (const child of parent.children ?? []) {
      await prisma.category.create({
        data: {
          ...
          nameEn: child.nameEn,
          status: CategoryStatus.ACTIVE,
          attributesSchema: schemaFor(child.slug) as Prisma.InputJsonValue,
        },
      });
    }
  }
...
function schemaFor(slug: string): { fields: AttributeField[] } {
  if (
    [
      "event-furniture",
      "tables",
  ...
  if (
    ["tents-and-shade", "tents", "parasols", "shade-structures"].includes(slug)
  ) {
    return schemas.tent;
  }
  if (
  ...
  if (
    ["microphones", "wireless-microphones", "wired-microphones"].includes(slug)
  ) {
  ...
  if (
    ["projectors", "presentation-equipment", "clickers", "adapters"].includes(
      slug,
    )
  ) {
  ...
  if (
    ["mobile-kitchen-equipment", "ovens", "grills", "cooktops"].includes(slug)
  ) {
