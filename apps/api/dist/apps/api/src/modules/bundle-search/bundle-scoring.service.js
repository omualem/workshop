...
const scoring_1 = require("@rental/scoring");
let BundleScoringService = class BundleScoringService {
    scoreBundle(bundle, allBundles, input, weights) {
        ...
        const reliabilityScore = this.clamp(concreteItems.reduce((sum, item) => sum + item.reliabilityScore, 0) / Math.max(1, concreteItems.length));
        const logisticsPenalty = bundle.totalDistanceKm * 0.12 + (bundle.pickupPointsCount - 1) * 1.6 + (bundle.lendersCount - 1) * 0.8;
        ...
        const availabilityScore = this.clamp(concreteItems.reduce((sum, item) => sum + item.availabilityFitScore, 0) /
            Math.max(1, concreteItems.length) +
            (bundle.exactAvailabilityFit ? 0.4 : -0.8));
        const productQualityScore = this.clamp(concreteItems.reduce((sum, item) => sum + item.qualityScore, 0) / Math.max(1, concreteItems.length));
        const stability = (0, scoring_1.computeStabilityAdjustedScore)({
            price: priceScore,
        ...
        const stabilityScore = this.clamp(10 - stability.imbalancePenalty - stability.lowScorePenalty + stability.bottleneckAdjustment / 2);
        return {
