...
exports.BundleGenerationService = void 0;
...
let BundleGenerationService = class BundleGenerationService {
    prisma;
    availabilityService;
    pricingService;
    lenderReliabilityService;
    constructor(prisma, availabilityService, pricingService, lenderReliabilityService) {
        this.prisma = prisma;
        this.availabilityService = availabilityService;
        this.pricingService = pricingService;
        this.lenderReliabilityService = lenderReliabilityService;
    }
    async findCandidatesPerSlot(input) {
        const result = {};
        const counts = {};
        for (const slot of input.requestedItems) {
            ...
            for (const listing of rawListings) {
                const distanceKm = (0, utils_1.haversineDistanceKm)({ lat: input.renterLocation.lat, lng: input.renterLocation.lng }, { lat: Number(listing.pickupLat), lng: Number(listing.pickupLng) });
                if (slot.constraints.distanceMaxKm && distanceKm > slot.constraints.distanceMaxKm) {
                    continue;
                }
                const available = await this.availabilityService.isListingAvailable(listing.id, slot.quantity, new Date(input.dateRange.startDate), new Date(input.dateRange.endDate), listing.inventoryCount);
                if (!available) {
                    continue;
                }
                const priceQuote = this.pricingService.computeListingPrice(listing, new Date(input.dateRange.startDate), new Date(input.dateRange.endDate), slot.quantity);
                ...
                const availabilityFitScore = await this.availabilityService.availabilityFragilityScore(listing.id, new Date(input.dateRange.startDate), new Date(input.dateRange.endDate));
                const prelimScore = reliabilityScore * 0.3 +
                    qualityScore * 0.2 +
    ...
    generateBundles(input, slotCandidates) {
        ...
        const walk = (index, acc) => {
            ...
            if (index === slots.length) {
                ...
                seen.add(dedupeKey);
                const pickupPoints = new Set(concreteItems.map((item) => `${item.listing.lenderId}:${item.listing.pickupLat}:${item.listing.pickupLng}`));
    ...
    async fetchListingsForSlot(slot) {
        ...
        return listings.filter((listing) => {
            ...
            if (slot.constraints.attributes) {
                return Object.entries(slot.constraints.attributes).every(([key, value]) => {
                    const attribute = listing.attributeValues.find((entry) => entry.attributeKey === key);
    ...
    computeListingQualityScore(listing) {
        const imageCountScore = Math.min(10, listing.media.length * 2);
        const descriptionScore = Math.min(2, listing.descriptionHe.length / 120) + Math.min(2, listing.descriptionEn.length / 120);
        const attributeScore = Math.min(2, listing.attributeValues.length / 3);
        const reviewAvg = listing.reviews.length > 0
        ...
        return Math.max(0, Math.min(10, imageCountScore * 0.25 + descriptionScore * 1.2 + attributeScore + ratingScore + conditionScore));
    }
};
exports.BundleGenerationService = BundleGenerationService;
exports.BundleGenerationService = BundleGenerationService = __decorate([
...
//# sourceMappingURL=bundle-generation.service.js.map