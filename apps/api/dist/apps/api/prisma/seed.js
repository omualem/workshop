...
const parentCategories = [
    { key: "events", slug: "events", nameHe: "אירועים", nameEn: "Events" },
    { key: "photography", slug: "photography", nameHe: "צילום", nameEn: "Photography" },
    { key: "audio", slug: "audio", nameHe: "אודיו", nameEn: "Audio" },
    { key: "outdoor", slug: "outdoor", nameHe: "חוץ", nameEn: "Outdoor" },
    { key: "power", slug: "power", nameHe: "חשמל וכוח", nameEn: "Power" },
];
...
async function main() {
    ...
    const childCategoryIds = new Map();
    const parentCategoryRows = parentCategories.map((parent, index) => {
    ...
    const childCategoryRows = categoryTemplates.map((category, index) => {
    ...
    renterNames.forEach((fullName, index) => {
        ...
        renterProfiles.push({
            ...
            verificationStatus: index % 4 === 0 ? client_1.VerificationStatus.VERIFIED : client_1.VerificationStatus.PENDING,
            preferences: {
                preferredLanguage: index % 5 === 0 ? "en" : "he",
                preferenceProfile: pick(["balanced", "cheapest", "mostReliable", "easiestPickup", "bestDateFit"]),
    ...
    lenders.forEach((lender, lenderIndex) => {
        ...
        if (lenderIndex % 3 === 0) {
            const guaranteedBundleSet = lender.archetype === "local"
    ...
    const bundleSearchRows = [];
    const bundleCandidateRows = [];
    const bundleCandidateItemRows = [];
    const savedSearchRows = [];
    const activeListings = listingMetas.filter((listing) => listing.listingStatus === client_1.ListingStatus.ACTIVE);
    ...
    searchScenarioTemplates.forEach((scenario, scenarioIndex) => {
        ...
        const drafts = scenario.strategies.map((strategy, strategyIndex) => buildCandidateDraft({
            scenario,
            strategy,
        ...
        bundleSearchRows.push({
            ...
            status: client_1.BundleSearchStatus.READY,
        });
...
function createSearchScenarioTemplates() {
    return [
        {
            key: "photo-balanced",
...
function createLenderMetrics(archetype, featured) {
    if (archetype === "cheap") {
        ...
        return {
            ...
            complaintRate,
            responseTimeScore,
            verificationLevel: bool(0.22) ? client_1.VerificationLevel.VERIFIED : client_1.VerificationLevel.BASIC,
            completedTransactionsCount: intBetween(18, 75),
            reliabilityScore: reliabilityFormula(averageRating, cancellationRate, lateReturnRate, complaintRate, responseTimeScore, client_1.VerificationLevel.BASIC, 28),
        };
    }
    if (archetype === "premium") {
        ...
        const verificationLevel = featured || bool(0.7) ? client_1.VerificationLevel.TRUSTED : client_1.VerificationLevel.VERIFIED;
        ...
        return {
            ...
            reliabilityScore: reliabilityFormula(averageRating, cancellationRate, lateReturnRate, complaintRate, responseTimeScore, verificationLevel, completedTransactionsCount),
        };
    }
    if (archetype === "local") {
        ...
        const verificationLevel = bool(0.35) ? client_1.VerificationLevel.TRUSTED : client_1.VerificationLevel.VERIFIED;
    ...
    const verificationLevel = bool(0.55) ? client_1.VerificationLevel.VERIFIED : client_1.VerificationLevel.BASIC;
...
function reliabilityFormula(averageRating, cancellationRate, lateReturnRate, complaintRate, responseTimeScore, verificationLevel, completedTransactionsCount) {
    const sampleBoost = Math.min(1.1, Math.log10(completedTransactionsCount + 10) / 2.2);
    const verificationBoost = verificationLevel === client_1.VerificationLevel.TRUSTED
        ? 0.9
        : verificationLevel === client_1.VerificationLevel.VERIFIED
            ? 0.45
            : 0.0;
    const raw = averageRating * 1.55 +
...
function createListingRecord(args) {
    ...
    category.attributes.forEach((attribute) => {
        attributeRows.push({
            ...
            attributeValue: pick(attribute.values),
        });
    });
    pricingRows.push({
        id: makeId(62, pricingRows.length + 1),
        listingId: id,
        ruleType: client_1.PricingRuleType.DURATION_DISCOUNT,
        ...
        listingId: id,
        ruleType: client_1.PricingRuleType.WEEKEND_ADJUSTMENT,
    ...
    if (qualityTier !== "low" || bool(0.5)) {
        pricingRows.push({
            id: makeId(62, pricingRows.length + 1),
            listingId: id,
            ruleType: client_1.PricingRuleType.SEASONAL_ADJUSTMENT,
...
function childCategoryIdsFromRuntime(categoryKey) {
    return runtimeCategoryIds.get(categoryKey);
}
const runtimeCategoryIds = new Map();
function chooseQualityTier(archetype) {
    if (archetype === "premium") {
        return weightedPick([
            { value: "high", weight: 0.68 },
            { value: "mid", weight: 0.28 },
            { value: "low", weight: 0.04 },
        ]);
    }
    if (archetype === "local") {
        return weightedPick([
            { value: "high", weight: 0.34 },
            { value: "mid", weight: 0.52 },
            { value: "low", weight: 0.14 },
        ]);
    }
    if (archetype === "cheap") {
        return weightedPick([
            { value: "high", weight: 0.08 },
    ...
    return weightedPick([
        ...
        { value: "mid", weight: 0.44 },
        { value: "low", weight: 0.36 },
    ]);
}
function choosePriceBand(archetype, categoryKey) {
    if (archetype === "premium") {
        return weightedPick([
            { value: "premium", weight: 0.72 },
            { value: "mid", weight: 0.28 },
        ]);
    }
    if (archetype === "cheap") {
        return weightedPick([
            { value: "cheap", weight: 0.78 },
    ...
    if (archetype === "local") {
        if (categoryKey === "chairs" || categoryKey === "tables") {
            return weightedPick([
                { value: "mid", weight: 0.75 },
                { value: "cheap", weight: 0.15 },
                { value: "premium", weight: 0.1 },
            ]);
        }
        return weightedPick([
            { value: "mid", weight: 0.62 },
            { value: "premium", weight: 0.22 },
            { value: "cheap", weight: 0.16 },
        ]);
    }
    return weightedPick([
        { value: "mid", weight: 0.42 },
        { value: "cheap", weight: 0.34 },
        { value: "premium", weight: 0.24 },
    ]);
}
function determineInventory(category, archetype) {
...
function chooseAvailabilityProfile(archetype) {
    if (archetype === "premium") {
        return weightedPick([
            { value: "open", weight: 0.42 },
            { value: "partial", weight: 0.4 },
            { value: "fragmented", weight: 0.15 },
            { value: "scarce", weight: 0.03 },
        ]);
    }
    if (archetype === "local") {
        return weightedPick([
            { value: "open", weight: 0.3 },
            { value: "partial", weight: 0.44 },
            { value: "fragmented", weight: 0.2 },
            { value: "scarce", weight: 0.06 },
        ]);
    }
    if (archetype === "cheap") {
        return weightedPick([
            { value: "open", weight: 0.18 },
            { value: "partial", weight: 0.34 },
            { value: "fragmented", weight: 0.31 },
            { value: "scarce", weight: 0.17 },
        ]);
    }
    return weightedPick([
        { value: "open", weight: 0.15 },
        { value: "partial", weight: 0.29 },
        { value: "fragmented", weight: 0.36 },
        { value: "scarce", weight: 0.2 },
    ]);
}
function chooseListingStatus(archetype, qualityTier) {
    ...
    if (bool(blockedChance)) {
        return client_1.ListingStatus.BLOCKED;
    }
    if (bool(pendingChance)) {
        return client_1.ListingStatus.PENDING_REVIEW;
    }
    return client_1.ListingStatus.ACTIVE;
}
function chooseCondition(archetype, qualityTier) {
    if (qualityTier === "high") {
        return weightedPick([
            { value: client_1.ListingCondition.NEW, weight: archetype === "premium" ? 0.52 : 0.18 },
            { value: client_1.ListingCondition.LIKE_NEW, weight: 0.48 },
        ]);
    }
    if (qualityTier === "mid") {
        return weightedPick([
            { value: client_1.ListingCondition.LIKE_NEW, weight: 0.36 },
            { value: client_1.ListingCondition.GOOD, weight: 0.54 },
            { value: client_1.ListingCondition.FAIR, weight: 0.1 },
        ]);
    }
    return weightedPick([
        { value: client_1.ListingCondition.GOOD, weight: 0.34 },
        { value: client_1.ListingCondition.FAIR, weight: 0.48 },
        { value: client_1.ListingCondition.HEAVY_USE, weight: 0.18 },
    ]);
}
function calculateQualityScore(args) {
    const base = args.archetype === "premium"
        ? 7.7
    ...
    const conditionBoost = args.condition === client_1.ListingCondition.NEW
        ? 0.8
        : args.condition === client_1.ListingCondition.LIKE_NEW
            ? 0.45
            : args.condition === client_1.ListingCondition.GOOD
                ? 0.1
                : args.condition === client_1.ListingCondition.FAIR
                    ? -0.45
                    : -0.9;
    return round2(clamp(base + qualityTierBoost + imageBoost + descriptionBoost + conditionBoost, 3.9, 9.8));
}
function buildHebrewDescription(category, lender, qualityTier) {
    const sentenceA = `${pick(category.detailsHe)}.`;
    const sentenceB = lender.archetype === "premium"
    ...
    const sentenceC = qualityTier === "high"
...
function buildEnglishDescription(category, lender, qualityTier) {
...
function generateAvailabilityBlocks(listingId, profile, inventoryCount, archetype) {
    const rows = [];
    ...
    if (profile === "open") {
        if (bool(archetype === "premium" ? 0.18 : 0.1)) {
            rows.push({
                ...
                status: client_1.AvailabilityBlockStatus.MAINTENANCE,
                quantity: 1,
    ...
    if (profile === "partial") {
        ...
        for (let index = 0; index < windowCount; index += 1) {
            ...
            rows.push({
                ...
                status: bool(0.76) ? client_1.AvailabilityBlockStatus.BOOKED : client_1.AvailabilityBlockStatus.BLOCKED,
    ...
    if (profile === "fragmented") {
        for (let week = 0; week < 8; week += 1) {
            ...
            rows.push({
                ...
                status: client_1.AvailabilityBlockStatus.BOOKED,
                quantity: baseQuantity,
    ...
    rows.push({
        ...
        status: client_1.AvailabilityBlockStatus.BLOCKED,
        quantity: Math.max(1, inventoryCount),
...
function pickWeightedCategory(lender) {
    const combined = [
...
function buildCandidateDraft(args) {
    ...
    const pickupPointsCount = unique(items.map((item) => `${item.listing.lenderId}:${item.listing.pickupAddressText}`)).length;
    const totalDistanceKm = round2(items.reduce((sum, item) => sum + haversineKm(renterLocation.lat, renterLocation.lng, item.listing.pickupLat, item.listing.pickupLng), 0));
    const totalPrice = round2(items.reduce((sum, item) => sum + item.listing.basePriceDaily * item.quantity * scenario.durationDays, 0));
    const reliabilityRaw = round2(average(uniqueLenders.map((lenderId) => lenders.find((candidate) => candidate.id === lenderId).reliabilityScore)));
    const qualityRaw = round2(average(items.map((item) => item.listing.qualityScore)));
    const availabilityRaw = round2(average(items.map((item) => availabilityProfileScore(item.listing.availabilityProfile))) -
        (strategy === "fragile" ? 2.0 : 0) -
        (strategy === "weakLink" ? 0.9 : 0));
    const logisticsRaw = round2(clamp(10.4 -
        totalDistanceKm * 0.32 -
        (pickupPointsCount - 1) * 1.2 +
        (uniqueLenders.length === 1 ? 1.8 : 0) +
        (items.every((item) => item.listing.deliverySupported) ? 0.45 : 0), 1, 10));
    const baseScores = [reliabilityRaw, logisticsRaw, clamp(availabilityRaw, 1, 10), qualityRaw];
    const stabilityRaw = round2(clamp(8.4 -
        standardDeviation(baseScores) * 1.15 -
        baseScores.filter((score) => score < 5.8).length * 0.85 +
        Math.min(...baseScores) * 0.18, 1, 10));
    return {
...
function rankCandidateDrafts(drafts, weights) {
    const minPrice = Math.min(...drafts.map((draft) => draft.rawPrice));
    const maxPrice = Math.max(...drafts.map((draft) => draft.rawPrice));
    return drafts
        .map((draft) => {
        const priceScore = maxPrice === minPrice ? 8.5 : round2(clamp(10 - ((draft.rawPrice - minPrice) / (maxPrice - minPrice)) * 8.2, 1.5, 10));
        const coreScores = [
            priceScore,
        ...
        const weightedMean = priceScore * weights.price +
        ...
        const stabilityScore = round2(clamp(draft.rawStability - stdDev * 0.18 - lowScores.length * 0.22 + Math.min(...coreScores) * 0.09, 1, 10));
        return {
            draft,
            priceScore,
            finalScore,
        ...
        const bundleRow = {
            ...
            debugData: {
                ...
                weightedMean: round2(row.priceScore * 0.2 +
        ...
        const itemRows = row.draft.items.map((item, itemIndex) => ({
...
function buildExplanation(draft, priceScore, stabilityScore) {
    const strengths = [
...
function chooseByStrategy(listings, strategy, renterLocation, slotIndex) {
    ...
    if (strategy === "sameLender") {
        return [...listings].sort((left, right) => haversineKm(renterLocation.lat, renterLocation.lng, left.pickupLat, left.pickupLng) -
            haversineKm(renterLocation.lat, renterLocation.lng, right.pickupLat, right.pickupLng))[0];
    }
    if (strategy === "fragile") {
    ...
    return [...listings].sort((left, right) => {
        const leftScore = left.qualityScore * 0.34 +
        ...
        const rightScore = right.qualityScore * 0.34 +
...
function availabilityProfileScore(profile) {
    if (profile === "open") {
        return 9.4;
    }
    if (profile === "partial") {
...
function findLenderForScenario(lenders, scenario, preferredArchetypes) {
    const slotCategories = scenario.slots.map((slot) => slot.categoryKey);
    return lenders.find((lender) => preferredArchetypes.includes(lender.archetype) &&
...
function createBookings(args) {
    ...
    const bookingAvailabilityBlocks = [];
    const completedBookingsForReview = [];
    const disputeEligibleBookings = [];
    const paymentBookings = [];
    const statusPlan = [
        ...Array.from({ length: 38 }, () => client_1.BookingStatus.COMPLETED),
        ...Array.from({ length: 10 }, () => client_1.BookingStatus.IN_PROGRESS),
        ...Array.from({ length: 8 }, () => client_1.BookingStatus.CONFIRMED),
        ...Array.from({ length: 8 }, () => client_1.BookingStatus.CANCELLED),
        ...Array.from({ length: 8 }, () => client_1.BookingStatus.REJECTED),
    ];
    statusPlan.forEach((status, index) => {
        ...
        const itemsForBooking = templateCategories.map((categoryKey, categoryIndex) => {
            ...
            const preferredArchetypes = status === client_1.BookingStatus.CANCELLED || status === client_1.BookingStatus.REJECTED
                ? ["cheap", "inconsistent"]
            ...
            const quantity = categoryKey === "chairs"
                ? intBetween(12, 36)
                : categoryKey === "tables"
                    ? intBetween(2, 6)
                    : categoryKey === "heaters"
        ...
        const totalPrice = round2(itemsForBooking.reduce((sum, entry) => sum + entry.listing.basePriceDaily * entry.quantity * duration, 0));
        const totalDeposit = round2(itemsForBooking.reduce((sum, entry) => sum + entry.listing.depositAmount * Math.min(entry.quantity, 3), 0));
        const logisticsSnapshot = round2(clamp(9.8 -
            unique(itemsForBooking.map((entry) => entry.listing.lenderId)).length * 1.1 -
            itemsForBooking.reduce((sum, entry) => sum + haversineKm(32.0722, 34.8117, entry.listing.pickupLat, entry.listing.pickupLng), 0) *
                0.18, 2, 10));
        const reliabilitySnapshot = round2(average(unique(itemsForBooking.map((entry) => entry.listing.lenderId)).map((lenderId) => lenders.find((candidate) => candidate.id === lenderId).reliabilityScore)));
        bookingRows.push({
            id: bookingId,
            renterId,
            ...
            paymentStatus: status === client_1.BookingStatus.COMPLETED
                ? client_1.PaymentStatus.CAPTURED
                : status === client_1.BookingStatus.IN_PROGRESS || status === client_1.BookingStatus.CONFIRMED
                    ? client_1.PaymentStatus.AUTHORIZED
                    : status === client_1.BookingStatus.REJECTED
                        ? client_1.PaymentStatus.NOT_REQUIRED
                        : status === client_1.BookingStatus.CANCELLED
                            ? client_1.PaymentStatus.REFUNDED
                            : client_1.PaymentStatus.PENDING,
            paymentReference: status === client_1.BookingStatus.REJECTED ? null : `pi_${bookingId.replace(/-/g, "").slice(0, 20)}`,
        });
        itemsForBooking.forEach((entry) => {
            bookingItemRows.push({
                ...
                pickupMethod: entry.listing.deliverySupported && bool(0.35) ? client_1.PickupMethod.DELIVERY : client_1.PickupMethod.PICKUP,
                pickupWindow: {
            ...
            if (status !== client_1.BookingStatus.CANCELLED && status !== client_1.BookingStatus.REJECTED) {
        ...
        if (status === client_1.BookingStatus.COMPLETED && completedBookingsForReview.length < 44) {
        ...
        if (status === client_1.BookingStatus.CANCELLED || status === client_1.BookingStatus.REJECTED) {
            disputeEligibleBookings.push({
                ...
                status: status === client_1.BookingStatus.CANCELLED ? client_1.DisputeStatus.UNDER_REVIEW : client_1.DisputeStatus.OPEN,
                reason: status === client_1.BookingStatus.CANCELLED
                    ? "Pickup changed after confirmation and renter requested review"
                    : "Listing was unavailable after request approval",
                resolutionNote: status === client_1.BookingStatus.REJECTED ? null : "Pending review by admin ops",
            });
        }
        if (status !== client_1.BookingStatus.REJECTED) {
            paymentBookings.push({
                bookingId,
                amount: totalPrice,
                status: status === client_1.BookingStatus.COMPLETED
                    ? "captured"
                    : status === client_1.BookingStatus.CANCELLED
                        ? "refunded"
                        : status === client_1.BookingStatus.IN_PROGRESS || status === client_1.BookingStatus.CONFIRMED
...
function createReviews(completedBookingsForReview, lenders) {
    return completedBookingsForReview.map((entry, index) => {
        const lender = lenders.find((candidate) => candidate.id === entry.revieweeUserId);
        const rating = entry.lenderArchetype === "premium"
            ? weightedPick([
                { value: 5, weight: 0.78 },
                { value: 4, weight: 0.22 },
            ])
        ...
        return {
            ...
            tags: reviewTags(entry.lenderArchetype, rating),
            createdAt: offsetDate(new Date(`${seedYear}-02-10T10:00:00.000Z`), index * 2, 12),
        };
    });
}
function createDisputes(disputes, adminId) {
...
function createPaymentPlaceholders(paymentBookings) {
    return paymentBookings.map((payment, index) => ({
...
function createNotifications(adminId, renterIds) {
    const rows = [];
    renterIds.slice(0, 10).forEach((renterId, index) => {
        rows.push({
            ...
            type: client_1.NotificationType.BUNDLE_RESULT,
    ...
    rows.push({
        ...
        userId: adminId,
        channel: client_1.NotificationChannel.IN_APP,
        type: client_1.NotificationType.ADMIN,
...
function createRankingConfigs(adminId) {
    return [
        {
            id: makeId(140, 1),
            presetKey: "balanced",
...
function createAuditLogs(adminId, lenders) {
    return [
        {
            id: makeId(150, 1),
            actorUserId: adminId,
...
function createFavoriteRows(renterIds, activeListings) {
    const rows = [];
    const used = new Set();
...
function reviewText(archetype, rating, displayName) {
    if (archetype === "premium") {
        return rating === 5
...
function reviewTags(archetype, rating) {
    if (archetype === "premium") {
...
async function createInChunks(rows, size, insert) {
    for (let index = 0; index < rows.length; index += size) {
        await insert(rows.slice(index, index + size));
    }
}
function createCoordinates(neighborhood, spreadMultiplier) {
    return {
...
function offsetDate(date, days, hour) {
...
function createLenderBio(displayNameHe, archetype, neighborhoodHe) {
    if (archetype === "premium") {
...
function explanationSummary(label, strengths, weaknesses) {
...
function explanationSummaryHe(labelHe, strengthsHe, weaknessesHe) {
...
function translateExplanation(value) {
    const dictionary = {
